...
//        }
        ...
        val basic = "Basic ${Utils.toBase64(AppState.session.token + ":" + AppState.session.serverAddress.removePrefix("https://").removePrefix("http://"))}"
        val state = SensorEventAPI(BuildConfig.HOST).sensorEventCreate(AppState.session.userId, sendTokenRequest, basic)
        LampLog.e(TAG, "Logout Done -  $state")
    ...
    private fun retrieveCurrentToken() {
        FirebaseInstanceId.getInstance().instanceId.addOnCompleteListener {
            ...
            Thread {
                ...
                val state = SensorEventAPI(BuildConfig.HOST).sensorEventCreate(AppState.session.userId, sendTokenRequest, basic)
                LampLog.e(TAG, "Notification Received -  $state")
